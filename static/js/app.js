...
(function () {
  ...
  proceedBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showEmailError(data.error || 'Ошибка при создании заказа');
        resetProceedBtn();
        return;
      }
      ...
      showEmailError('Ошибка соединения. Проверьте интернет.');
      resetProceedBtn();
    }
  });
  ...
  function showStatusSection(type) {
    ...
    if (type === 'pending') statusPending.classList.remove('hidden');
